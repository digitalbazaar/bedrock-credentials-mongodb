...
Store.prototype.get = function(actor, id, callback) {
  ...
  async.waterfall([
    function(callback) {
      logger.warning('FIXME: fix credential get permission check');
      brPermission.checkPermission(
        /*actor*/null, PERMISSIONS.CREDENTIAL_ACCESS, {
          resource: id,
          translate: ['issuer', 'recipient']
        }, callback);
    },
    function(callback) {
      self.collection.findOne({id: database.hash(id)}, {}, callback);
    },
    function(record, callback) {
      ...
      callback(null, database.decode(record.credential),
        database.decode(record.meta));
    }
  ], callback);
};
  ...
  async.auto({
    ...
    clean: ['find', function(callback, results) {
      ...
      async.forEachSeries(results.find, function(record, callback) {
        ...
        logger.warning('FIXME: fix credential getAll permission check');
        brPermission.checkPermission(
          /*actor*/null, PERMISSIONS.CREDENTIAL_ACCESS, {
            resource: credential,
            translate: ['issuer', 'recipient'],
...
Store.prototype.compose = function(
  ...
  var identity = {
    '@context': bedrock.config.constants.CREDENTIALS_CONTEXT_V1_URL,
    id: recipient
  };
  async.auto({
    checkPermission: function(callback) {
      ...
      async.until(function() {return done;}, function(callback) {
        cursor.nextObject(function(err, record) {
          ...
          if(_.intersection(properties, assertions).length > 0) {
            bedrock.jsonld.addProperty(identity, 'credential', cred);
          }
          callback();
        });
      }, callback);
