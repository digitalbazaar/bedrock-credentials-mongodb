...
function Store() {
  ...
  this.events.removeAllListeners('maxListenersPassed');
}
 ...
 * @param [options] the options to use:
 *          [meta] custom meta data to include when storing the credential.
  ...
  async.auto({
    checkPermission: function(callback) {
      brPermission.checkPermission(
        ...
        {resource: credential, translate: ['issuer', 'claim']}, callback);
    },
    emitInsert: ['checkPermission', function(callback) {
      credential = bedrock.util.clone(credential);
      event = {
        actor: actor,
      ...
      self.events.emit('insert', event, callback);
    }],
    insert: ['emitInsert', function(callback) {
      var referenceIdHash;
      credential.sysStatus = 'active';
      if(credential.referenceId) {
      ...
      if('meta' in options) {
        record.meta = _.assign({}, options.meta, record.meta);
      }
    ...
    emitPostInsert: ['insert', function(callback, results) {
      ...
      event.credential = bedrock.util.clone(record.credential);
      event.meta = bedrock.util.clone(record.meta);
      bedrock.events.emit('postInsert', event, callback);
    }]
          ...
          actor, PERMISSIONS.CREDENTIAL_ACCESS, {
            ...
            if(err) {
              logger.error(new BedrockError(
                'credentials-mongodb.getAll', 'PermissionDenied', {
                  actor: actor.id,
...
Store.prototype.count = function(actor, query, options, callback) {
  ...
  async.auto({
    checkPermission: function(callback) {
      brPermission.checkPermission(
        actor, PERMISSIONS.CREDENTIAL_ACCESS, callback);
    },
...
/**
 * Marks a Credential as deleted.
 *
 * @param actor the Identity performing the action.
 * @param credentialId the Credential ID.
 * @param callback(err) called once the operation completes.
 */
Store.prototype.remove = function(actor, credentialId, callback) {
  var self = this;
  async.auto({
    find: function(callback) {
      self.collection.find({id: database.hash(credentialId)})
        .limit(1).next(function(err, result) {
          ...
          if(!result) {
            return callback(new BedrockError(
              'Could not remove credential. Credential not found.',
              'NotFound',
              {httpStatusCode: 404, credentialId: credentialId, public: true}
            ));
          }
    ...
    checkPermission: ['find', function(callback, results) {
      var credential = results.find.credential;
      brPermission.checkPermission(
        actor, PERMISSIONS.CREDENTIAL_REMOVE,
        {resource: credential, translate: 'claim'}, callback);
    }],
    update: ['checkPermission', function(callback) {
      var now = Date.now();
      self.collection.update(
        {id: database.hash(credentialId)},
        {$set: {'credential.sysStatus': 'deleted', 'meta.updated': now}},
    ...
    callback(null, results.update.result);
  });
};
